...
Attach each item to a location along a familiar, fixed route. Recall = mentally
walking the route. Best for **ordered** material and for anything longer than a
handful of items.
...
- Stations should be **visually distinct** and roughly evenly spaced. Don't use ten
  near-identical cubicles; use the door, the window, the weird stain, the plant.
...
station is independent. Use linking for quick, casual lists; loci for high stakes.

Example (key, umbrella, passport): a giant brass **key** unlocks a storm cloud that
dumps rain, so you open an **umbrella**, and the umbrella flips inside-out to reveal
a **passport** stapled to every rib.

---

## 3. The Major System (numbers → words)

Numbers are pure abstraction, so first convert digits to consonant sounds, then pad
with vowels to make a memorable word, then make an image of the word. Standard
mapping (by sound, not spelling):

| Digit | Sound(s) | Memory hook |
|------:|----------|-------------|
| 0 | s, z | "zero" starts with z |
| 1 | t, d | one downstroke |
| 2 | n | two legs on an n |
| 3 | m | three legs on an m |
| 4 | r | "four" ends in r |
| 5 | l | Roman L = 50 |
| 6 | j, sh, ch, soft g | a 6 looks like a flipped J |
| 7 | k, hard g | a K holds two 7s |
| 8 | f, v | cursive f ≈ 8 |
| 9 | p, b | 9 is a mirrored P |

Vowels and w/h/y are free filler. So 31 → m,t → "mat"; 92 → p,n → "pen"; 42 → r,n →
"rain." A year like 1942 → t,r,b,n → "turban." Place the word-image in a palace and
you've stored the number.
...
the whole route. Trade-off: capacity is fixed by how many pegs you've built.
...
Use the **keyword method**: find a word in the learner's own language that *sounds*
like the foreign word, then build an image linking that keyword to the meaning.

Spanish *caballo* (horse) sounds like "cob-eye-oh" → picture a horse eating a giant
corn **cob** that pokes it in the **eye**. French *poubelle* (trash can) → "poo-bell"
→ a trash can with a bell on top that rings whenever something stinky goes in.

The sillier and more sensory the link, the faster the word becomes automatic. Once
it's automatic, the scaffold falls away and you just *know* the word.
