/**
 * Tests for the AzureStorageAdapter.
 *
 * We mock `globalThis.fetch` to simulate the GraphVault server proxy without
 * needing a real server or Azure account. We also mock `sessionStorage` so
 * the token and server URL lookups work in Node.
 */

import assert from 'node:assert/strict';
import { test, before, after, beforeEach } from 'node:test';
import { AzureStorageAdapter, AZURE_VAULT_OBJECT_KEY } from './azureAdapter';
...
const TEST_TOKEN = 'test-bearer-token-azure';
const TEST_SERVER_URL = 'http://127.0.0.1:4000';
const EXPECTED_PROXY_URL = `${TEST_SERVER_URL}/v1/storage/azure/object/${AZURE_VAULT_OBJECT_KEY}`;
...
test('isAvailable() returns false without sessionStorage', () => {
  removeSessionStorage();
  const adapter = new AzureStorageAdapter();
...
test('load() returns empty array when vault object does not exist in Azure (404)', async () => {
  const adapter = new AzureStorageAdapter();
  const notes = await adapter.load();
  assert.deepEqual(notes, []);
});

test('save() puts a vault document to the Azure proxy URL', async () => {
  const adapter = new AzureStorageAdapter();
...
test('clear() deletes the vault object from Azure', async () => {
  const adapter = new AzureStorageAdapter();
...
test('adapter has correct id and label', () => {
  const adapter = new AzureStorageAdapter();
  assert.equal(adapter.id, 'azure');
  assert.equal(typeof adapter.label, 'string');
  assert.ok(adapter.label.toLowerCase().includes('azure'));
});

test('AZURE_VAULT_OBJECT_KEY is the expected filename', () => {
  assert.equal(AZURE_VAULT_OBJECT_KEY, 'graphvault-vault.json');
});
